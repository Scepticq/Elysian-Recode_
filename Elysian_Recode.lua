...
do
	local LOG_PATH = "nl\\elysian\\error_log.txt"
	local function _ts()
		local t = 0
		pcall(function() t = globals.realtime or globals.curtime or 0 end)
	...
	local function _append(line)
		pcall(function()
			pcall(files.create_folder, "nl\\elysian")
			local prev = ""
			pcall(function() prev = files.read(LOG_PATH) or "" end)
			if #prev > 120000 then prev = prev:sub(-60000) end
			files.write(LOG_PATH, prev .. line .. "\n")
		end)
		pcall(function()
			print_raw(line)
		end)
	...
	function elysian_log(msg)
		_append(string.format("[%s] %s", _ts(), tostring(msg)))
	end
	function elysian_log_err(where, err)
		_append(string.format("[%s] ERROR @ %s | %s", _ts(), tostring(where), tostring(err)))
	end
	function elysian_safe(where, fn)
		...
		if not ok then
			elysian_log_err(where, err)
		end
		return ok
	end
	elysian_log("=== script load start ===")
	pcall(function()
		local last_hb = 0
		events.render(function()
			local t = globals.realtime or 0
			if t - last_hb < 300 then return end
			last_hb = t
			local n = 0
			pcall(function() collectgarbage("count"); n = collectgarbage("count") end)
			elysian_log(string.format("heartbeat ok | mem=%.0fKB | tick=%s", n, tostring(globals.tickcount)))
		end)
	end)
end
...
local slot_0_discord_rpc = {
	enabled = true,
	last_update = 0,
	update_interval = 15
}

function slot_0_discord_rpc.init()
	pcall(function()
		if _G.discord_rpc then
			_G.discord_rpc:set({
				state = "NeverLose CS:GO",
				details = "elysian recode",
				largeImageKey = "neverlose",
				largeImageText = "elysian recode",
				buttons = {
					{label = "Discord", url = "https://discord.gg/kTHUpjVQPV"}
				}
			})
		end
	end)
...
function slot_0_discord_rpc.update()
	local current_time = globals.realtime
	if current_time - slot_0_discord_rpc.last_update < slot_0_discord_rpc.update_interval then
		return
	end
	
	slot_0_discord_rpc.last_update = current_time
	
	pcall(function()
		if _G.discord_rpc then
			local lp = entity.get_local_player()
			local status = "In Menu"
			
			if lp and lp:is_alive() then
				status = "In Game"
				pcall(function()
					local h = lp.get_health and lp:get_health() or lp.m_iHealth
					if h then status = "In Game (" .. tostring(h) .. " HP)" end
				end)
			end
			
			_G.discord_rpc:set({
				state = "NeverLose CS:GO",
				details = "Playing with elysian recode",
				smallImageKey = "elysian_icon",
				smallImageText = status,
				largeImageKey = "neverlose",
				largeImageText = "elysian recode",
				startTimestamp = math.floor(globals.realtime)
			})
		end
	end)
end

pcall(function() slot_0_discord_rpc.init() end)

events.render(function()
	if slot_0_discord_rpc.enabled then
		slot_0_discord_rpc.update()
	end
end)
...
slot_0_13_0 = {
	message = function(arg_4_0, arg_4_1)
		print_raw(string.format("\a%selysian recode\aDEFAULT  %s", ui.get_style()["Link Active"]:to_hex(), arg_4_1))
	end,
	error = function(arg_5_0, arg_5_1)
		print_raw(string.format("\a%selysian recode\aDEFAULT  \aFF3E3EFF%s", ui.get_style()["Link Active"]:to_hex(), arg_5_1))
	end
}
...
function slot_0_58_0.is_active(arg_45_0, arg_45_1)
	...
	local name = arg_45_1 and tostring(arg_45_1):lower() or ""

	if name == "double tap" or name == "dt" then
		local ok, v = pcall(function() return slot_0_11_0.rage.main.double_tap:get() end)
		if ok then return v and true or false end
	end
	if name == "hide shots" or name == "hs" then
		local ok, v = pcall(function() return slot_0_11_0.rage.main.hide_shots:get() end)
		if ok then return v and true or false end
	end
...
slot_0_84_13.export = slot_0_85_14:button("  export  ", nil, true)
slot_0_84_13.telegram = slot_0_85_14:button("  \f<paper-plane>\r  ", function()
	pcall(function()
		if panorama and panorama.SteamOverlayAPI and panorama.SteamOverlayAPI.OpenExternalBrowserURL then
			panorama.SteamOverlayAPI.OpenExternalBrowserURL("https://t.me/elysianremake")
		end
	end)
	pcall(function()
		utils.console_exec("open https://t.me/elysianremake")
	end)
end, true)
slot_0_84_13.discord = slot_0_85_14:button("  \f<discord>\r  ", function()
	pcall(function()
		if panorama and panorama.SteamOverlayAPI and panorama.SteamOverlayAPI.OpenExternalBrowserURL then
			panorama.SteamOverlayAPI.OpenExternalBrowserURL("https://discord.gg/539pTEMz5F")
		end
	end)
	pcall(function()
		utils.console_exec("open https://discord.gg/539pTEMz5F")
	end)
end, true)
slot_0_84_13.stats1 = slot_0_85_14:label("shots 0  |  hits 0  |  miss 0")
slot_0_84_13.stats2 = slot_0_85_14:label("kills 0  |  deaths 0  |  hs 0%")
slot_0_84_13.stats3 = slot_0_85_14:label("success 0%")
slot_0_84_13.stats_reset = slot_0_85_14:button("  reset stats  ", nil, true)
...
slot_0_83_8_chinahat = {
	switch = slot_0_79_9:switch("\v\f<umbrella>\r  china hat"):depend({
		slot_0_78_7,
		slot_0_76_6
	})
}
slot_0_84_10_chinahat = slot_0_83_8_chinahat.switch:create()
slot_0_83_8_chinahat.color = slot_0_84_10_chinahat:color_picker(slot_0_60_1("color"), color(255, 255, 255, 255)):depend({
	slot_0_83_8_chinahat.switch,
	true
})
slot_0_83_8_chinahat.rgb = slot_0_84_10_chinahat:switch(slot_0_60_1("rgb mod"), false):depend({
	slot_0_83_8_chinahat.switch,
	true
})
slot_0_83_8_chinahat.speed = slot_0_84_10_chinahat:slider(slot_0_60_1("speed"), 1, 100, 5):depend({
	slot_0_83_8_chinahat.switch,
	true
}, {
	slot_0_83_8_chinahat.rgb,
	true
})
slot_0_83_8_chinahat.size = slot_0_84_10_chinahat:slider(slot_0_60_1("size"), 4, 24, 10, 0.5, "u"):depend({
	slot_0_83_8_chinahat.switch,
	true
})
slot_0_83_8_chinahat.height = slot_0_84_10_chinahat:slider(slot_0_60_1("height"), 2, 20, 8, 0.5, "u"):depend({
	slot_0_83_8_chinahat.switch,
	true
})
slot_0_83_8_chinahat.fill_a = slot_0_84_10_chinahat:slider(slot_0_60_1("fill alpha"), 0, 255, 50):depend({
	slot_0_83_8_chinahat.switch,
	true
})
slot_0_83_8_chinahat.style = slot_0_84_10_chinahat:combo(slot_0_60_1("style"), {
	"default",
	"solid",
	"glass",
	"glow",
...
slot_0_83_8_chinahat.thirdperson_only = slot_0_84_10_chinahat:switch(slot_0_60_1("thirdperson only"), true):depend({
	slot_0_83_8_chinahat.switch,
	true
})
slot_0_81_6.china_hat = slot_0_83_8_chinahat


slot_0_83_8_circle = {
	switch = slot_0_80_6:switch("\v\f<circle>\r  jump circle"):depend({
		slot_0_78_7,
		slot_0_76_6
	})
}
slot_0_84_10_circle = slot_0_83_8_circle.switch:create()
slot_0_83_8_circle.color = slot_0_84_10_circle:color_picker(slot_0_60_1("color"), color(150, 195, 255, 255)):depend({
	slot_0_83_8_circle.switch,
	true
})
slot_0_83_8_circle.duration = slot_0_84_10_circle:slider(slot_0_60_1("duration"), 0.1, 2.0, 0.8, 0.1):depend({
	slot_0_83_8_circle.switch,
	true
})
slot_0_83_8_circle.max_radius = slot_0_84_10_circle:slider(slot_0_60_1("max radius"), 10, 100, 50, 1):depend({
	slot_0_83_8_circle.switch,
	true
})
slot_0_81_6.jump_circle = slot_0_83_8_circle

slot_0_83_8_tr = {
	switch = slot_0_80_6:switch("\v\f<wave-pulse>\r  player trails"):depend({
		slot_0_78_7,
		slot_0_76_6
	})
}
slot_0_84_10_tr = slot_0_83_8_tr.switch:create()
slot_0_83_8_tr.color = slot_0_84_10_tr:color_picker(slot_0_60_1("color"), color(150, 195, 255, 180)):depend({
	slot_0_83_8_tr.switch,
	true
})
slot_0_83_8_tr.length = slot_0_84_10_tr:slider(slot_0_60_1("trail length"), 10, 100, 50, 1):depend({
	slot_0_83_8_tr.switch,
	true
})
slot_0_83_8_tr.width = slot_0_84_10_tr:slider(slot_0_60_1("trail width"), 1, 20, 5, 1):depend({
	slot_0_83_8_tr.switch,
	true
})
slot_0_83_8_tr.fade = slot_0_84_10_tr:switch(slot_0_60_1("fade effect"), true):depend({
	slot_0_83_8_tr.switch,
	true
})

slot_0_81_6.player_trails = slot_0_83_8_tr

slot_0_elysian_custom_model = slot_0_80_6:switch("\v\f<person>\r  custom model", false):depend({
	slot_0_78_7,
	slot_0_76_6
})
slot_0_elysian_cm_group = slot_0_elysian_custom_model:create()
slot_0_elysian_cm_path = slot_0_elysian_cm_group:input("path", "models/player/custom_player/legacy/tm_phoenix.mdl"):depend({ slot_0_elysian_custom_model, true })
slot_0_elysian_cm_name = slot_0_elysian_cm_group:input("save name", "my model"):depend({ slot_0_elysian_custom_model, true })
slot_0_elysian_cm_saves = {}
pcall(function()
	local raw = files.read("nl\\elysian\\models.json")
	if raw and #raw > 2 then
		local ok, data = pcall(json.parse, raw)
		if ok and type(data) == "table" then
			slot_0_elysian_cm_saves = data
		end
	end
end)
slot_0_elysian_cm_combo_items = { "(path field)" }
for iter_cm = 1, #slot_0_elysian_cm_saves do
	local n = slot_0_elysian_cm_saves[iter_cm].name or ("model " .. iter_cm)
	slot_0_elysian_cm_combo_items[#slot_0_elysian_cm_combo_items + 1] = n
end
slot_0_elysian_cm_pick = slot_0_elysian_cm_group:combo("saved models", slot_0_elysian_cm_combo_items, "(path field)"):depend({ slot_0_elysian_custom_model, true })
slot_0_elysian_cm_save_btn = slot_0_elysian_cm_group:button("save current path", function() end):depend({ slot_0_elysian_custom_model, true })
slot_0_elysian_cm_del_btn = slot_0_elysian_cm_group:button("delete selected", function() end):depend({ slot_0_elysian_custom_model, true })


slot_0_83_8_sp = {
	switch = slot_0_80_6:switch("\v\f<bolt-lightning>\r  spark impact"):depend({
		slot_0_78_7,
		slot_0_76_6
	})
}
slot_0_84_10_sp = slot_0_83_8_sp.switch:create()
slot_0_83_8_sp.self_only = slot_0_84_10_sp:switch(slot_0_60_1("only my shots"), true):depend({ slot_0_83_8_sp.switch, true })
slot_0_83_8_sp.color = slot_0_84_10_sp:color_picker(slot_0_60_1("spark color"), color(255, 220, 80, 255)):depend({ slot_0_83_8_sp.switch, true })
slot_0_83_8_sp.count = slot_0_84_10_sp:slider(slot_0_60_1("count"), 8, 80, 35):depend({ slot_0_83_8_sp.switch, true })
slot_0_83_8_sp.speed = slot_0_84_10_sp:slider(slot_0_60_1("spread"), 100, 900, 420):depend({ slot_0_83_8_sp.switch, true })
slot_0_83_8_sp.gravity = slot_0_84_10_sp:slider(slot_0_60_1("gravity"), 0, 1500, 700):depend({ slot_0_83_8_sp.switch, true })
slot_0_83_8_sp.life = slot_0_84_10_sp:slider(slot_0_60_1("duration"), 2, 20, 9):depend({ slot_0_83_8_sp.switch, true })
slot_0_83_8_sp.tail = slot_0_84_10_sp:slider(slot_0_60_1("tail length"), 1, 25, 8):depend({ slot_0_83_8_sp.switch, true })
slot_0_83_8_sp.thick = slot_0_84_10_sp:slider(slot_0_60_1("thickness"), 1, 6, 2):depend({ slot_0_83_8_sp.switch, true })
slot_0_83_8_sp.flash = slot_0_84_10_sp:switch(slot_0_60_1("center flash"), true):depend({ slot_0_83_8_sp.switch, true })
slot_0_83_8_sp.flash_sz = slot_0_84_10_sp:slider(slot_0_60_1("flash size"), 2, 20, 8):depend({ slot_0_83_8_sp.switch, true }, { slot_0_83_8_sp.flash, true })
slot_0_83_8_sp.gradient = slot_0_84_10_sp:switch(slot_0_60_1("gradient"), true):depend({ slot_0_83_8_sp.switch, true })
slot_0_83_8_sp.perspective = slot_0_84_10_sp:switch(slot_0_60_1("3d perspective"), true):depend({ slot_0_83_8_sp.switch, true })
slot_0_83_8_sp.collide = slot_0_84_10_sp:switch(slot_0_60_1("surface bounce"), true):depend({ slot_0_83_8_sp.switch, true })
slot_0_83_8_sp.bounce = slot_0_84_10_sp:slider(slot_0_60_1("bounce strength"), 0, 80, 35):depend({ slot_0_83_8_sp.switch, true }, { slot_0_83_8_sp.collide, true })
slot_0_81_6.spark_impact = slot_0_83_8_sp

slot_0_83_8_xh = {
	switch = slot_0_80_6:switch("\v\f<star-of-life>\r  israel crosshair"):depend({
		slot_0_78_7,
		slot_0_76_6
	})
}
slot_0_84_10_xh = slot_0_83_8_xh.switch:create()
slot_0_83_8_xh.color = slot_0_84_10_xh:color_picker(slot_0_60_1("color"), color(0, 80, 200, 230)):depend({ slot_0_83_8_xh.switch, true })
slot_0_83_8_xh.size = slot_0_84_10_xh:slider(slot_0_60_1("size"), 6, 40, 16):depend({ slot_0_83_8_xh.switch, true })
slot_0_83_8_xh.speed = slot_0_84_10_xh:slider(slot_0_60_1("spin speed"), 0, 20, 5):depend({ slot_0_83_8_xh.switch, true })
slot_0_83_8_xh.gap = slot_0_84_10_xh:slider(slot_0_60_1("center gap"), 0, 20, 2):depend({ slot_0_83_8_xh.switch, true })
slot_0_81_6.spin_crosshair = slot_0_83_8_xh

slot_0_83_8_snow = {
	switch = slot_0_80_6:switch("\v\f<snowflake>\r  map snow"):depend({
		slot_0_78_7,
		slot_0_76_6
	})
}
slot_0_84_10_snow = slot_0_83_8_snow.switch:create()
slot_0_83_8_snow.color = slot_0_84_10_snow:color_picker(slot_0_60_1("color"), color(220, 235, 255, 200)):depend({ slot_0_83_8_snow.switch, true })
slot_0_83_8_snow.count = slot_0_84_10_snow:slider(slot_0_60_1("count"), 10, 80, 35):depend({ slot_0_83_8_snow.switch, true })
slot_0_83_8_snow.radius = slot_0_84_10_snow:slider(slot_0_60_1("radius"), 200, 1500, 700):depend({ slot_0_83_8_snow.switch, true })
slot_0_83_8_snow.size = slot_0_84_10_snow:slider(slot_0_60_1("size"), 1, 8, 2):depend({ slot_0_83_8_snow.switch, true })
slot_0_83_8_snow.speed = slot_0_84_10_snow:slider(slot_0_60_1("fall speed"), 1, 40, 12):depend({ slot_0_83_8_snow.switch, true })
slot_0_83_8_snow.wind = slot_0_84_10_snow:slider(slot_0_60_1("wind"), 0, 30, 8):depend({ slot_0_83_8_snow.switch, true })
slot_0_81_6.map_snow = slot_0_83_8_snow
...
slot_0_90_7_c = slot_0_90_7.enabled:create()
slot_0_90_7.mode = slot_0_90_7_c:combo(slot_0_60_1("mode"), {
	"elysian",
	"custom",
	"custom + stars",
	"static"
})
slot_0_90_7.custom_text = slot_0_90_7_c:input(slot_0_60_1("custom text"), "elysian")
slot_0_90_7.star_style = slot_0_90_7_c:combo(slot_0_60_1("star style"), {
	"✦",
	"★",
	"☆",
	"✧",
...
slot_0_90_7.speed = slot_0_90_7_c:slider(slot_0_60_1("anim speed"), 5, 40, 13, 1, "cs")
...
function slot_0_78_5(arg_156_0)
	if type(arg_156_0) == "table" then
		return arg_156_0
	end
	if type(arg_156_0) ~= "string" then
		return nil
	end
	...
	if var_156_0 and var_156_1 ~= nil then
		return var_156_1
	end
	...
	local ok2, parsed = pcall(function()
		return json.parse(arg_156_0)
	end)
	if ok2 and parsed ~= nil then
		return parsed
	end

	local ok3, parsed3 = pcall(function()
		local raw = slot_0_2_0.decode(arg_156_0)
		return json.parse(raw)
	end)
	if ok3 and parsed3 ~= nil then
		return parsed3
	end
...
function elysian_cfg_count(tbl)
	local n = 0
	local function walk(x, d)
		if d > 14 or type(x) ~= "table" then return end
		for _, v in pairs(x) do
			n = n + 1
			if type(v) == "table" then walk(v, d + 1) end
		end
	end
	walk(tbl, 0)
	return n
...
function elysian_pack_control_value(val)
...
function elysian_set_control(ctrl, val)
	if ctrl == nil or val == nil then return false end
	if type(ctrl) ~= "table" and type(ctrl) ~= "userdata" then return false end
	if type(ctrl.set) ~= "function" then return false end
	...
	if type(val) == "table" and val[1] ~= nil then
		ok = pcall(function()
			ctrl:set(color(val[1], val[2] or 0, val[3] or 0, val[4] or 255))
		end)
		return ok
	end
	return false
...
function elysian_pack_block(block)
	local o = {}
	if type(block) ~= "table" then return o end
	for k, v in pairs(block) do
		if type(k) == "string" then
			pcall(function()
				if (type(v) == "table" or type(v) == "userdata") and type(v.get) == "function" then
					o[k] = elysian_pack_control_value(v:get())
				end
			end)
		end
	end
...
function slot_0_elysian_extras_save()
	local out = { flick = {}, visuals = {} }
	pcall(function()
		local flick = slot_0_59_0.aim and slot_0_59_0.aim.flick
		if flick then out.flick = elysian_pack_block(flick) end
	end)
	pcall(function()
		local vis = slot_0_59_0.visuals or slot_0_81_6
		if not vis then return end
		for _, key in ipairs({ "china_hat", "player_trails", "jump_circle", "aspect_ratio" }) do
			out.visuals[key] = elysian_pack_block(vis[key])
		end
	end)
	return out
end

function slot_0_elysian_extras_load(data)
	if type(data) ~= "table" then return end
	pcall(function()
		local flick = slot_0_59_0.aim and slot_0_59_0.aim.flick
		if flick and data.flick then elysian_apply_block(flick, data.flick) end
	end)
	pcall(function()
		local vis = slot_0_59_0.visuals or slot_0_81_6
		if not vis or type(data.visuals) ~= "table" then return end
		for key, block in pairs(data.visuals) do
			if vis[key] then elysian_apply_block(vis[key], block) end
		end
	end)
end

function elysian_snapshot_pui()
	local data = nil
	pcall(function()
		data = slot_0_1_0.save()
	end)
...
function elysian_apply_pui(data)
	if type(data) ~= "table" then return false end
	local ok = false
	pcall(function()
		slot_0_1_0.load(data)
		ok = true
	end)
	pcall(function()
		slot_0_1_0.load(data, false)
		ok = true
	end)
	local function walk(node, src, depth)
		if depth > 12 or type(node) ~= "table" or type(src) ~= "table" then return end
		for k, val in pairs(src) do
			if type(k) == "string" and k:sub(1, 2) ~= "__" then
				local child = node[k]
				if child ~= nil then
					if elysian_set_control(child, val) then
					elseif type(child) == "table" and type(val) == "table" then
	...
	pcall(function() walk(slot_0_59_0, data, 0) end)
	return ok
end

function elysian_build_package()
	local pkg = {
		v = 3,
		author = (slot_0_105_15 and slot_0_105_15()) or "",
		time = 0,
		nonce = tostring(globals.realtime or 0) .. "_" .. (function() local ok,v=pcall(function() return utils.random_int(10000,99999) end) return tostring(ok and v or 12345) end)()
	}
	pcall(function() pkg.time = common.get_unixtime() end)
	pkg.pui = elysian_snapshot_pui()
	pcall(function()
		local f = slot_0_65_0 or slot_0_121_2
		if f then pkg.builder = f() end
	end)
	pcall(function()
		local f = slot_0_67_0 or slot_0_124_2
		if f then pkg.defensive = f() end
	end)
	pcall(function()
		pkg.extras = slot_0_elysian_extras_save()
	end)
	pcall(function()
		if slot_0_82_3 then pkg.positions = slot_0_82_3() end
	end)
	return pkg
end

function elysian_apply_package(pkg)
	if type(pkg) ~= "table" then
		return false, "not a table"
	end

	local pui_data = pkg.pui
	if not pui_data and pkg.config then
		local c = pkg.config
		if type(c) == "string" and slot_0_78_5 then
			c = slot_0_78_5(c)
		end
		pui_data = c
	end
	...
	if type(pui_data) == "table" then
		elysian_apply_pui(pui_data)
		utils.execute_after(0.05, function() elysian_apply_pui(pui_data) end)
		utils.execute_after(0.2, function() elysian_apply_pui(pui_data) end)
	end

	local builder = pkg.builder or pkg.builder_plain
	if type(builder) == "string" and slot_0_78_5 then
		builder = slot_0_78_5(builder)
	end
	if type(builder) == "table" then
		local apply = slot_0_66_0 or slot_0_122_2
		if apply then
			pcall(apply, builder)
			utils.execute_after(0.05, function() pcall(apply, builder) end)
			utils.execute_after(0.2, function() pcall(apply, builder) end)
		end
	end

	local defensive = pkg.defensive or pkg.defensive_plain
	if type(defensive) == "string" and slot_0_78_5 then
		defensive = slot_0_78_5(defensive)
	end
	if type(defensive) == "table" then
		local apply = slot_0_68_0 or slot_0_125_2
		if apply then
			pcall(apply, defensive)
			utils.execute_after(0.05, function() pcall(apply, defensive) end)
			utils.execute_after(0.2, function() pcall(apply, defensive) end)
		end
	end

	local extras = pkg.extras or pkg.extras_plain
	if type(extras) == "table" then
		slot_0_elysian_extras_load(extras)
		utils.execute_after(0.05, function() slot_0_elysian_extras_load(extras) end)
		utils.execute_after(0.25, function() slot_0_elysian_extras_load(extras) end)
	end

	if type(pkg.positions) == "table" and slot_0_83_4 then
		pcall(slot_0_83_4, pkg.positions)
	elseif type(pkg.positions) == "string" and slot_0_78_5 and slot_0_83_4 then
		local pos = slot_0_78_5(pkg.positions)
		if type(pos) == "table" then pcall(slot_0_83_4, pos) end
	end

	return true, "ok"
end

function elysian_encode_pkg(pkg)
	if slot_0_77_4 then
		local ok, s = pcall(slot_0_77_4, pkg)
		if ok and type(s) == "string" and #s > 8 then return s end
	end
	local ok, s = pcall(json.stringify, pkg)
	if ok then return s end
	return nil
end

function elysian_decode_pkg(str)
	if type(str) == "table" then return str end
	if type(str) ~= "string" or str == "" then return nil end
	if slot_0_78_5 then
		local ok, pkg = pcall(slot_0_78_5, str)
		if ok and type(pkg) == "table" then return pkg end
	end
	local ok, pkg = pcall(json.parse, str)
	if ok and type(pkg) == "table" then return pkg end
	return nil
end

function elysian_clipboard_get()
	local s = nil
	pcall(function()
		if slot_0_3_0 and slot_0_3_0.get then s = slot_0_3_0.get() end
	end)
	if not s or s == "" then
		pcall(function()
			if clipboard and clipboard.get then s = clipboard.get() end
		end)
	end
	return s
end

function elysian_clipboard_set(str)
	local ok = false
	pcall(function()
		if slot_0_3_0 and slot_0_3_0.set then
			slot_0_3_0.set(str)
			ok = true
		end
	end)
	pcall(function()
		if not ok and clipboard and clipboard.set then
			clipboard.set(str)
			ok = true
		end
	end)
...
function slot_0_100_12(arg_182_0, arg_182_1)
	return elysian_apply_pui(arg_182_0)
end

function slot_0_101_13()
	local var_195_0 = slot_0_59_0.info.presets.name:get()
	if not var_195_0 then return end
	local var_195_1 = slot_0_71_2[var_195_0]
	if not var_195_1 then return end
	local var_195_2 = var_195_1
	if type(var_195_1) == "string" then
		var_195_2 = slot_0_78_5(var_195_1)
	end
	if type(var_195_2) == "table" then
		local _ = var_195_2.author
	end
end
...
function slot_0_104_14()
	local name = slot_0_59_0.info.presets.name:get()
	if not name or name:gsub("%s", "") == "" then
		slot_0_64_0.new({ "invalid name" })
		return
	end
	if name == slot_0_73_2 then return end

	local packed = slot_0_71_2[name]
	if not packed then
		slot_0_64_0.new({ "config not found" })
		return
	end

	local pkg = elysian_decode_pkg(packed)
	if type(pkg) ~= "table" then
		slot_0_64_0.new({ "corrupt package" })
		return
	end

	local ok, msg = elysian_apply_package(pkg)
	if ok then
		slot_0_13_0:message("loaded " .. name)
		slot_0_64_0.new({ "loaded  ", name })
	else
		slot_0_64_0.new({ "load failed" })
		pcall(function() slot_0_13_0:error(tostring(msg)) end)
	end
end
...
function slot_0_106_15()
	local name = slot_0_59_0.info.presets.name:get()
	if not name or name:gsub("%s", "") == "" then
		slot_0_64_0.new({ "name cannot be empty" })
		return
	end
	if name == slot_0_73_2 then return end

	for _, def in ipairs(slot_0_74_2 or {}) do
		if def == name then
			slot_0_64_0.new({ "enter a new name to create config" })
			return
		end
	end

	local pkg = elysian_build_package()
	local keys = elysian_cfg_count(pkg.pui) + elysian_cfg_count(pkg.builder) + elysian_cfg_count(pkg.extras)
	if keys < 1 then
		slot_0_13_0:error("save empty — open AA/visuals once, then save")
	end

	slot_0_103_13(name, pkg)
	slot_0_13_0:message("saved " .. name .. " keys≈" .. tostring(keys))
	slot_0_64_0.new({ "saved  ", name })
end

function slot_0_107_14()
	local name = slot_0_59_0.info.presets.name:get()
	if not name or name:gsub("%s", "") == "" then
		slot_0_64_0.new({ "enter a name first" })
		return
	end
	if name == slot_0_73_2 then return end
	for _, def in ipairs(slot_0_74_2 or {}) do
		if def == name then
			slot_0_64_0.new({ "cannot overwrite default" })
			return
		end
	end
	if slot_0_71_2[name] then
		slot_0_64_0.new({ "config already exists" })
		return
	end
	...
	slot_0_103_13(name, pkg)
	pcall(slot_0_101_13)
	slot_0_13_0:message("created " .. name)
	slot_0_64_0.new({ "created  ", name })
end

function slot_0_109_18(mode)
	mode = mode or "normal"
	local pr = slot_0_59_0 and slot_0_59_0.info and slot_0_59_0.info.presets
	if not pr then return end
	local is_confirm = mode == "export" or mode == "import"
	pcall(function()
		if pr.load then pr.load:visibility(not is_confirm) end
		if pr.save then pr.save:visibility(not is_confirm) end
		if pr.create then pr.create:visibility(not is_confirm) end
		if pr.delete then pr.delete:visibility(not is_confirm) end
		if pr.export then pr.export:visibility(not is_confirm) end
		if pr.import then pr.import:visibility(not is_confirm) end
		if pr.telegram then pr.telegram:visibility(not is_confirm) end
		if pr.confirm then pr.confirm:visibility(is_confirm) end
		if pr.cancel then pr.cancel:visibility(is_confirm) end
		if pr.list then pr.list:visibility(not is_confirm) end
		if pr.name then pr.name:visibility(not is_confirm) end
	end)
end
...
function slot_0_110_18()
	local ok, err = pcall(function()
		local pkg = elysian_build_package()
		local str = elysian_encode_pkg(pkg)
		if not str or #tostring(str) < 8 then
			error("empty encode")
		end
		if not elysian_clipboard_set(str) then
			error("clipboard set failed")
		end
		slot_0_13_0:message("export len " .. tostring(#tostring(str)))
		slot_0_64_0.new({ "copied settings" })
	end)
	if not ok then
		pcall(function() slot_0_13_0:error("export fail") end)
		slot_0_64_0.new({ "export failed" })
	end
end

function slot_0_111_23()
	local ok, err = pcall(function()
		local clip = elysian_clipboard_get()
		if not clip or tostring(clip):gsub("%s", "") == "" then
			slot_0_64_0.new({ "clipboard is empty" })
			return
		end
		local pkg = elysian_decode_pkg(clip)
		if type(pkg) ~= "table" then
			slot_0_64_0.new({ "invalid import data" })
			return
		end
		local applied = elysian_apply_package(pkg)
		if applied then
			slot_0_13_0:message("imported from clipboard")
			slot_0_64_0.new({ "imported settings" })
		else
			slot_0_64_0.new({ "import apply failed" })
		end
	end)
	if not ok then
		pcall(function() slot_0_13_0:error("import fail") end)
		slot_0_64_0.new({ "import failed" })
	end
end
...
function slot_0_114_19()
	...
	if not var_212_0 or var_212_0:gsub("%s", "") == "" then return end
	if var_212_0 == slot_0_73_2 then return end
	if not slot_0_71_2[var_212_0] then return end
	slot_0_13_0:message("deleted " .. var_212_0)
	slot_0_64_0.new({ "deleted  ", var_212_0 })
...
function slot_0_89_0.update_yaw(arg_374_0, arg_374_1, arg_374_2, arg_374_3)
	local var_374_pitch = arg_374_2.pitch and arg_374_2.pitch:get() or "Down"
	arg_374_1.pitch = var_374_pitch
	arg_374_1.yaw = "backward"

	local var_374_0 = false
	local var_374_at_target_enabled = arg_374_2.yaw_base and arg_374_2.yaw_base:get() == "at target"

	if var_374_at_target_enabled then
		local var_374_1 = entity.get_local_player()
		local var_374_2 = var_374_1 and var_374_1:get_origin()
		local var_374_3 = math.huge
		local var_374_has_target = false
		...
		slot_0_10_0(true, false, function(arg_375_0)
			...
			local hp_ok, hp_val = pcall(function()
				if arg_375_0.get_health then
					return arg_375_0:get_health()
				end
				return arg_375_0.m_iHealth
			end)
			if not hp_ok or not hp_val or hp_val <= 0 then
				return
			end
			...
			if var_375_1 < var_374_3 then
				var_374_3 = var_375_1
				var_374_0 = true
				var_374_has_target = true
			end
		end)
		
		if not var_374_has_target then
			var_374_0 = false
		end
	end
	...
	if var_376_1 == "jitter" then
		...
		if var_376_4 == "static" then
			...
			if var_376_5 > 0 then
				...
				local var_376_current_cycle = arg_376_0.tickcount % (var_376_6 * 2)

				if arg_376_0.choked_commands == 0 then
					slot_0_95_4 = var_376_6 > var_376_current_cycle
				end
...
events.createmove(function(arg_391_0)
	if slot_0_59_0.aim and slot_0_59_0.aim.flick and slot_0_59_0.aim.flick.enabled and slot_0_59_0.aim.flick.enabled:get() then
		return
	end
...
slot_0_100_5 = "Verdana"
slot_0_101_6 = ""
slot_0_102_6 = "Verdana"
...
events.render(function()
		...
		for iter_470_0, iter_470_1 in ipairs(slot_470_14_0) do
			if iter_470_1.value and iter_470_1.value ~= 0 then
				slot_470_12_0[#slot_470_12_0 + 1] = {
					name = iter_470_1.name,
					value = iter_470_1.value,
					mode = iter_470_1.mode,
					active = iter_470_1.active
				}
			end
		end
	end
	...
	pcall(function()
		local function set_active_by_name(lname, is_on)
			for i = 1, #slot_470_12_0 do
				local n = slot_470_12_0[i].name
				if n and n:lower() == lname then
					slot_470_12_0[i].active = is_on and true or false
					return true
				end
			end
			return false
		...
		local dt_on = false
		pcall(function()
			dt_on = slot_0_11_0.rage.main.double_tap:get() and true or false
		end)
		set_active_by_name("double tap", dt_on)

		local hs_on = false
		pcall(function()
			hs_on = slot_0_11_0.rage.main.hide_shots:get() and true or false
		end)
		set_active_by_name("hide shots", hs_on)

		local fd_on = false
		pcall(function()
			fd_on = slot_0_11_0.antiaim.misc.fake_duck:get() and true or false
		end)
		set_active_by_name("fake duck", fd_on)

		local fs_on = false
		pcall(function()
			fs_on = slot_0_11_0.antiaim.angles.freestanding:get() and true or false
		end)
		if not set_active_by_name("freestanding", fs_on) then
			set_active_by_name("freestand", fs_on)
		end

		local dmg_on = false
		pcall(function()
			if slot_0_58_0 and slot_0_58_0.is_active then
				dmg_on = slot_0_58_0:is_active("min. damage")
			end
		end)
		pcall(function()
			if not dmg_on and slot_0_11_0.rage.selection.minimum_damage_global then
				dmg_on = slot_0_11_0.rage.selection.minimum_damage_global:get() and true or false
			end
		end)
		set_active_by_name("min. damage", dmg_on)

		local function ensure_row(lname, is_on)
			for i = 1, #slot_470_12_0 do
				local n = slot_470_12_0[i].name
				if n and n:lower() == lname then
					slot_470_12_0[i].active = is_on and true or false
			...
			slot_470_12_0[#slot_470_12_0 + 1] = {
				...
				value = 1,
				mode = "toggle",
				active = is_on and true or false
			}
		end

		local dt_on2 = false
		pcall(function() dt_on2 = slot_0_11_0.rage.main.double_tap:get() and true or false end)
		ensure_row("double tap", dt_on2)

		local hs_on2 = false
		pcall(function() hs_on2 = slot_0_11_0.rage.main.hide_shots:get() and true or false end)
		local had_hs = false
		for i = 1, #slot_470_12_0 do
			local n = slot_470_12_0[i].name
			if n and n:lower() == "hide shots" then had_hs = true break end
		end
		if had_hs or hs_on2 then
			ensure_row("hide shots", hs_on2)
		end
	end)
	...
	if slot_470_41_0 then
		...
		for iter_470_14, iter_470_15 in ipairs(slot_0_115_13) do
			...
			slot_470_76_0 = slot_470_74_0 and 1 or 0.7
...
slot_0_jump_circles = {}
slot_0_jump_last_on_ground = true

events.createmove(function(arg_jump_cm)
	local local_player = entity.get_local_player()
	if not local_player then return end
	
	local is_on_ground = bit.band(local_player.m_fFlags or 0, 1) == 1
	
	if slot_0_jump_last_on_ground and not is_on_ground then
		local origin = local_player:get_origin()
		if origin and slot_0_59_0.visuals.jump_circle and slot_0_59_0.visuals.jump_circle.switch:get() then
			table.insert(slot_0_jump_circles, {
	...
	slot_0_jump_last_on_ground = is_on_ground
end)

events.render(function()
	local circle = slot_0_59_0.visuals.jump_circle
	if not circle or not circle.switch:get() then
		return
	end

	local current_time = globals.realtime
	local segments = 48
	for i = #slot_0_jump_circles, 1, -1 do
		local c = slot_0_jump_circles[i]
		local elapsed = current_time - c.start_time
		local duration = circle.duration:get()

		if elapsed > duration then
			table.remove(slot_0_jump_circles, i)
		else
			local progress = elapsed / duration
			local radius = circle.max_radius:get() * progress
			local circle_col = circle.color:get()
			local alpha = math.floor(circle_col.a * (1 - progress))
			if alpha > 2 then
				local col = color(circle_col.r, circle_col.g, circle_col.b, alpha)
				local col_soft = color(circle_col.r, circle_col.g, circle_col.b, math.floor(alpha * 0.35))
				local ox, oy, oz = c.pos.x, c.pos.y, c.pos.z + 1
				local prev = nil
				local first = nil
				for s = 0, segments do
					local ang = (s / segments) * math.pi * 2
					local wx = ox + math.cos(ang) * radius
					local wy = oy + math.sin(ang) * radius
					local scr = render.world_to_screen(vector(wx, wy, oz))
					if scr and prev then
						render.line(prev, scr, col)
						local wx2 = ox + math.cos(ang) * (radius + 1.5)
						local wy2 = oy + math.sin(ang) * (radius + 1.5)
						local scr2 = render.world_to_screen(vector(wx2, wy2, oz))
						if scr2 then
						end
					end
					if scr then
						if not first then first = scr end
						prev = scr
					else
						prev = nil
					end
				...
				for s = 0, segments do
					local ang = (s / segments) * math.pi * 2
					local wx = ox + math.cos(ang) * (radius * 0.92)
					local wy = oy + math.sin(ang) * (radius * 0.92)
					local scr = render.world_to_screen(vector(wx, wy, oz))
					if scr and prev then
						render.line(prev, scr, col_soft)
					end
					if scr then prev = scr else prev = nil end
				end
			end
		end
	end
...
slot_0_trail_history = {}
slot_0_trail_max_points = 100

events.render(function()
	local trails = slot_0_59_0.visuals.player_trails
	
	if not trails or not trails.switch:get() then
		slot_0_trail_history = {}
		return
	end
	...
	table.insert(slot_0_trail_history, {
		pos = vector(origin.x, origin.y, origin.z),
		time = globals.realtime
	})
	
	local max_length = trails.length:get()
	while #slot_0_trail_history > max_length do
		table.remove(slot_0_trail_history, 1)
	end
	
	local current_time = globals.realtime
	for i = #slot_0_trail_history, 1, -1 do
		if current_time - slot_0_trail_history[i].time > 2.0 then
			table.remove(slot_0_trail_history, i)
		else
			break
		end
	end
	
	local trail_color = trails.color:get()
	local trail_width = trails.width:get()
	local use_fade = trails.fade:get()
	
	if #slot_0_trail_history < 2 then
		return
	end

	local function clamp_a(a)
		return math.max(0, math.min(255, math.floor(a + 0.5)))
	end

	local function glow_line(x1, y1, x2, y2, r, g, b, a)
		if a < 3 then return end
		local dx, dy = x2 - x1, y2 - y1
		local len = math.sqrt(dx * dx + dy * dy)
		if len < 0.5 then return end
		local inv = 1 / len
		local nx, ny = -dy * inv, dx * inv
		local fx1, fy1 = math.floor(x1 + 0.5), math.floor(y1 + 0.5)
		local fx2, fy2 = math.floor(x2 + 0.5), math.floor(y2 + 0.5)

		local function layer(mul, dist, bright)
			local aa = clamp_a(a * mul)
			if aa < 2 then return end
			local cr = clamp_a(r + (bright or 0))
			local cg = clamp_a(g + (bright or 0))
			local cb = clamp_a(b + (bright or 0))
			local c = color(cr, cg, cb, aa)
			local ox = math.floor(nx * dist + 0.5)
			local oy = math.floor(ny * dist + 0.5)
			if dist > 0 then
				render.line(vector(fx1 - ox, fy1 - oy), vector(fx2 - ox, fy2 - oy), c)
				render.line(vector(fx1 + ox, fy1 + oy), vector(fx2 + ox, fy2 + oy), c)
			else
				render.line(vector(fx1, fy1), vector(fx2, fy2), c)
			end
		end
	...
	for i = 1, #slot_0_trail_history - 1 do
		local point1 = slot_0_trail_history[i]
		local point2 = slot_0_trail_history[i + 1]
		local screen1 = render.world_to_screen(point1.pos)
		local screen2 = render.world_to_screen(point2.pos)
		if screen1 and screen2 then
			local alpha = trail_color.a
			if use_fade then
				alpha = math.floor(trail_color.a * (i / #slot_0_trail_history))
			end
			local a2 = clamp_a(alpha * (0.7 + trail_width * 0.04))
			glow_line(screen1.x, screen1.y, screen2.x, screen2.y, trail_color.r, trail_color.g, trail_color.b, a2)
		end
	end
end)
...
events.createmove(function(arg_553_1)
end)
...
slot_0_111_5 = {
	...
	{
		"пузатость не вывозит",
		"?"
	},
	{
		"лучше бы гонял с elysian recode",
		"пузатость"
	},
	{
		"Ремейк elysian от скептика запенил",
		"шавка"
	},
	{
		"бро это ремейк elysian от скептика",
		"?"
	},
	{
		"А если бы играл с elysian recode такого бы не было"
	},
	{
		"bro why u not used elysian recode"
	},
	{
		"Неверлуз безумие"
	},
...
function slot_0_118_4()
	local death_phrases = {
		"ofc body u fkn nn xd",
		"ебаный школьник не понимает что ему повезло",
		"лаки"
	}
	
	local random_phrase = death_phrases[math.random(1, #death_phrases)]
	local var_573_0 = 0.1
...
events.player_death(function(arg_578_0)
	...
	if var_578_3 == var_578_1 and var_578_2 ~= var_578_1 then
		...
		if not var_578_5 and not var_578_6 then
			slot_0_118_4()
		end
...
function slot_0_114_4(arg_580_0)
	...
	pcall(function()
		if common.set_clantag then
			common.set_clantag(arg_580_0)
		end
	end)
	pcall(function()
		if panorama then
			panorama.SteamOverlayAPI.SetClanTag(arg_580_0, arg_580_0)
		end
	end)
end
...
function slot_0_115_4()
	...
	pcall(function()
		if panorama then
			panorama.SteamOverlayAPI.SetClanTag("", "")
		end
	end)
end
...
function elysian_set_clantag_improved(tag)
	local success = false
	...
	if not success then
		pcall(function()
			if panorama and panorama.SteamOverlayAPI then
				panorama.SteamOverlayAPI.SetClanTag(tag, tag)
				success = true
			end
		end)
	end
...
elysian_set_clantag_improved(slot_0_0_0)

events.render(function()
	local var_586_0 = slot_0_59_0.misc.clantag
	if not var_586_0 then return end
	if not var_586_0.enabled:get() then
		if slot_0_116_4 then
			...
			slot_0_120_3 = ""
			slot_0_121_3 = {}
			slot_0_cl_last_key = nil
		end
		return
	end

	local mode = "elysian"
	pcall(function() mode = var_586_0.mode:get() end)
	local custom = "elysian"
	pcall(function() custom = var_586_0.custom_text:get() or "elysian" end)
	if custom == "" then custom = "elysian" end
	local star = "✦"
	pcall(function() star = var_586_0.star_style:get() or "✦" end)
	local spd = 0.13
	pcall(function()
		if var_586_0.speed then spd = (var_586_0.speed:get() or 13) / 100 end
	end)

	local key = mode .. "|" .. custom .. "|" .. star
	if slot_0_cl_last_key ~= key then
		slot_0_cl_last_key = key
		slot_0_121_3 = {}
		slot_0_118_3 = 0
		slot_0_120_3 = ""
		if mode == "static" then
			slot_0_121_3[1] = custom
		elseif mode == "custom" then
			for i = 1, #custom do
				slot_0_121_3[#slot_0_121_3 + 1] = custom:sub(1, i)
			end
			for i = #custom - 1, 1, -1 do
				slot_0_121_3[#slot_0_121_3 + 1] = custom:sub(1, i)
			end
			if #slot_0_121_3 == 0 then slot_0_121_3[1] = custom end
		elseif mode == "custom + stars" then
			slot_0_121_3[1] = star .. " " .. custom
			for i = 1, #custom do
				slot_0_121_3[#slot_0_121_3 + 1] = custom:sub(1, i - 1) .. star .. custom:sub(i)
			end
			slot_0_121_3[#slot_0_121_3 + 1] = custom .. " " .. star
			for i = #custom, 1, -1 do
				slot_0_121_3[#slot_0_121_3 + 1] = custom:sub(1, i - 1) .. star .. custom:sub(i)
			end
		else
			local base = slot_0_111_4 or "elysian recode"
			slot_0_121_3[1] = star .. " " .. base
			for i = 1, #base do
				slot_0_121_3[#slot_0_121_3 + 1] = base:sub(1, i - 1) .. star .. base:sub(i)
			end
			slot_0_121_3[#slot_0_121_3 + 1] = base .. " " .. star
		end
		slot_0_122_3 = #slot_0_121_3
	end
	...
	if mode == "static" then
		local tag = slot_0_121_3[1] or custom
		if tag ~= slot_0_120_3 then
			slot_0_120_3 = tag
			elysian_set_clantag_improved(tag)
			slot_0_114_4(tag)
		end
		return
	end

	if slot_0_117_4 >= spd then
		slot_0_117_4 = slot_0_117_4 - spd
		if slot_0_122_3 < 1 then return end
		slot_0_118_3 = slot_0_118_3 % slot_0_122_3 + 1
		local var_586_1 = slot_0_121_3[slot_0_118_3]
		if var_586_1 and var_586_1 ~= slot_0_120_3 then
			slot_0_120_3 = var_586_1
			elysian_set_clantag_improved(var_586_1)
...
elysian_safe("flick_init", function()
	elysian_log("flick_init")
	if not slot_0_83_6 then elysian_log("flick skip: no misc tab"); return end

	local flick = {
		enabled = slot_0_83_6:switch("\v\f<bolt>\r  flick exploit", false)
	}
	local fl = flick.enabled:create()
	flick.hidden_yaw = fl:slider(slot_0_60_1("hidden yaw"), -180, 180, -70)
	flick.pitch_value = fl:slider(slot_0_60_1("pitch flick value"), -89, 89, 89)
	flick.indicator = fl:switch(slot_0_60_1("indicator"), true)
	flick.ind_color = fl:color_picker(slot_0_60_1("indicator color"), color(255, 50, 50, 255)):depend({ flick.indicator, true })

	slot_0_59_0.aim = slot_0_59_0.aim or {}
	slot_0_59_0.aim.flick = flick

	local ref = {
		aa_tog = ui.find("Aimbot", "Anti Aim", "Angles", "Enabled"),
		offset = ui.find("Aimbot", "Anti Aim", "Angles", "Yaw", "Offset"),
		y_modif = ui.find("Aimbot", "Anti Aim", "Angles", "Yaw Modifier"),
		ymod_off = ui.find("Aimbot", "Anti Aim", "Angles", "Yaw Modifier", "Offset"),
	...
	local active = false
	local function ovr(r, v)
		if r then pcall(function() r:override(v) end) end
	end
	local function clr(r)
		if r then pcall(function() r:override() end) end
	end

	local function apply(cmd)
		ovr(ref.aa_tog, true)
		pcall(function() rage.antiaim:inverter(false) end)
		ovr(ref.offset, 5)
		ovr(ref.y_modif, "Offset")
		ovr(ref.ymod_off, 1)
		ovr(ref.desync, true)
		ovr(ref.des_opts, {})
		ovr(ref.l_lim, 42)
		...
		ovr(ref.fs, false)
		ovr(ref.hidden, true)
		pcall(function() rage.antiaim:override_hidden_pitch(flick.pitch_value:get()) end)
		pcall(function() rage.antiaim:override_hidden_yaw_offset(flick.hidden_yaw:get()) end)
		ovr(ref.dt_lag, "Always On")
		ovr(ref.os_opts, "Break LC")
		if cmd then
			cmd.force_defensive = ((cmd.command_number or 0) % 7) == 0
		end
	end

	local function reset()
		for _, r in pairs(ref) do clr(r) end
	...
	events.createmove(function(cmd)
		if not flick.enabled:get() then
			if active then reset(); active = false end
			return
		end
		local lp = entity.get_local_player()
	...
	events.render(function()
		if not flick.enabled:get() or not flick.indicator:get() or not active then return end
		local lp = entity.get_local_player()
		if not lp or not lp:is_alive() then return end
		local ss = render.screen_size()
		render.text(1, vector(ss.x * 0.5, ss.y * 0.5 + 28), flick.ind_color:get(), "c", "FLICK")
	end)
...
pcall(function()
	if not slot_0_83_6 then return end
	local body = {
		enabled = slot_0_83_6:switch("\v\f<crosshairs>\r  body aim if lethal", false)
	}
	local bc = body.enabled:create()
	body.enemy_autosniper = bc:switch(slot_0_60_1("body if enemy scar/g3"), true)
	body.lethal = bc:switch(slot_0_60_1("body if lethal"), true)
	body.safety = bc:slider(slot_0_60_1("safety margin hp"), 0, 15, 2)
	body.indicator = bc:switch(slot_0_60_1("indicator"), true)

	slot_0_59_0.aim = slot_0_59_0.aim or {}
	slot_0_59_0.aim.body_lethal = body

	local body_ref = ui.find("Aimbot", "Ragebot", "Safety", "Body Aim")
	local DMG = {
		[7]={33,23},[8]={33,29},[10]={30,21},[13]={30,21},[16]={33,29},[60]={33,29},
		[39]={30,21},[9]={115,115},[11]={80,70},[38]={80,70},[40]={88,88},
		[1]={53,47},[64]={54,48},[4]={29,21},[32]={29,21},[61]={29,21},[36]={28,21},
		[30]={33,24},[3]={35,28},[2]={33,26},[63]={32,24},
	}

	local function clr()
		pcall(function() if body_ref then body_ref:override() end end)
	end
	local function force_body()
		pcall(function() if body_ref then body_ref:override("Force") end end)
		pcall(function() if body_ref then body_ref:override(true) end end)
	end
	local function weapon_id(ent)
		local id = nil
		pcall(function()
			local w = ent:get_player_weapon()
			if not w then return end
			if type(w.get_weapon_index) == "function" then id = w:get_weapon_index()
			else id = w.m_iItemDefinitionIndex end
		end)
		return tonumber(id)
	end
	local function is_scar_or_g3(id)
		return id == 38 or id == 11
	end
	local function read_hp_armor(ent)
		local hp, armor = 100, 0
		pcall(function()
			if type(ent.get_health) == "function" then hp = ent:get_health()
			else hp = ent.m_iHealth end
		end)
		pcall(function() armor = ent.m_ArmorValue or 0 end)
		return tonumber(hp) or 100, (tonumber(armor) or 0) > 0
	end
	local function our_body_dmg(wid, has_armor)
		local row = wid and DMG[wid]
		if row then return has_armor and row[2] or row[1] end
		return has_armor and 21 or 27
	end

	local last_state = false
	events.createmove(function()
		if not body.enabled:get() then
			if last_state then clr(); last_state = false end
			return
		end
		...
		pcall(function()
			...
			local alive = true
			pcall(function() alive = threat:is_alive() end)
			if not alive then return end
			if body.enemy_autosniper:get() then
				if is_scar_or_g3(weapon_id(threat)) then want = true end
			end
			if not want and body.lethal:get() then
				local hp, has_armor = read_hp_armor(threat)
				local dmg = our_body_dmg(weapon_id(me), has_armor)
				local margin = body.safety:get() or 0
				if hp > 0 and hp <= dmg - margin then want = true
				elseif hp > 0 and hp <= dmg then want = true end
			end
		end)
		if want then force_body(); last_state = true
		else
			if last_state then clr() end
			last_state = false
		end
	end)
	events.render(function()
		if not body.enabled:get() or not body.indicator:get() or not last_state then return end
		local ss = render.screen_size()
		render.text(1, vector(ss.x * 0.5, ss.y * 0.5 + 42), color(80, 220, 120, 220), "c", "BODY")
	end)
	events.shutdown(function() clr() end)
end)

elysian_safe("pui.setup", function() slot_0_1_0.setup(slot_0_59_0) end)
elysian_log("pui.setup done")
...
function slot_0_127_2()
	...
	for iter_603_0, iter_603_1 in ipairs(slot_0_126_2 or {}) do
...
events.shutdown(function()
	pcall(function()
		if _G.discord_rpc then
			_G.discord_rpc:clear()
		end
	end)
	slot_0_discord_rpc.enabled = false
end)

pcall(function()
local elysian_heavy = {
	tick = 0, side = false, phase = 0, spin = 0, chaos_seed = 0,
	body_flip = false, seq_idx = 1, delay_cnt = 0, exploit_was = false,
	seq_delay = {1, 2, 1, 3, 2, 1, 4, 2}
}

function elysian_heavy_is_air(lp)
	...
	if not on_ground or math.abs(vz) > 80 then return true end
	return false
end

function elysian_heavy_dt_active()
	local ok, dt = pcall(function() return slot_0_11_0.rage.main.double_tap:get() end)
	if ok and dt then return true end
	local ok2, hs = pcall(function() return slot_0_11_0.rage.main.hide_shots:get() end)
	if ok2 and hs then return true end
	local ok3, ex = pcall(function() return rage.exploit:get() end)
	if ok3 and type(ex) == "number" and ex > 0 then return true end
	return false
end

function elysian_heavy_apply(cmd)
	local lp = entity.get_local_player()
	if not lp or not lp:is_alive() then return end
	if not elysian_heavy_dt_active() then
		elysian_heavy.exploit_was = false
		return
	end
	if not elysian_heavy_is_air(lp) then
		elysian_heavy.exploit_was = false
		return
	end
	elysian_heavy.exploit_was = true

	local duck = (lp.m_flDuckAmount or 0) > 0.5
	local choked = cmd.choked_commands or 0
	local sending = choked == 0

	if sending then
		elysian_heavy.tick = elysian_heavy.tick + 1
		elysian_heavy.delay_cnt = elysian_heavy.delay_cnt + 1
		local need = elysian_heavy.seq_delay[elysian_heavy.seq_idx] or 2
		if elysian_heavy.delay_cnt >= need then
			elysian_heavy.delay_cnt = 0
			elysian_heavy.side = not elysian_heavy.side
			elysian_heavy.seq_idx = elysian_heavy.seq_idx % #elysian_heavy.seq_delay + 1
			elysian_heavy.body_flip = not elysian_heavy.body_flip
		end
		elysian_heavy.phase = (elysian_heavy.phase + 1) % 16
		elysian_heavy.spin = elysian_heavy.spin + utils.random_float(17, 47)
		if elysian_heavy.spin > 180 then elysian_heavy.spin = elysian_heavy.spin - 360 end
		if elysian_heavy.tick % 5 == 0 then
			elysian_heavy.chaos_seed = utils.random_float(-180, 180)
		end
	end

	local base_l = duck and -38 or -52
	local base_r = duck and 41 or 57
	local yaw = elysian_heavy.side and base_r or base_l
	local ways = duck and {-45, 0, 48, -22, 33} or {-58, -12, 18, 52, -35, 8, 41}
	local way_i = (elysian_heavy.phase % #ways) + 1
	yaw = yaw + ways[way_i] * 0.35
	yaw = yaw + elysian_heavy.chaos_seed * 0.22
	yaw = yaw + math.sin(elysian_heavy.spin * 0.0174533) * 28
	if sending then
		yaw = yaw + utils.random_float(-9, 9)
	else
		yaw = yaw + utils.random_float(-3, 3)
	end
	while yaw > 180 do yaw = yaw - 360 end
	while yaw < -180 do yaw = yaw + 360 end

	pcall(function()
		slot_0_11_0.antiaim.angles.enabled:override(true)
		slot_0_11_0.antiaim.angles.pitch:override("Down")
		slot_0_11_0.antiaim.angles.yaw:override("Backward")
		slot_0_11_0.antiaim.angles.yaw_base:override("At Target")
		slot_0_11_0.antiaim.angles.yaw_add:override(math.floor(yaw + 0.5))
		slot_0_11_0.antiaim.angles.yaw_modifier:override("Disabled")
		slot_0_11_0.antiaim.angles.modifier_offset:override(0)
		slot_0_11_0.antiaim.angles.body_yaw:override(true)
		slot_0_11_0.antiaim.angles.inverter:override(elysian_heavy.body_flip)
		slot_0_11_0.antiaim.angles.left_limit:override(60)
		slot_0_11_0.antiaim.angles.right_limit:override(60)
		slot_0_11_0.antiaim.angles.options:override({"Jitter"})
		slot_0_11_0.antiaim.angles.freestanding:override(false)
		slot_0_11_0.antiaim.angles.disable_yaw_modifiers:override(true)
		slot_0_11_0.antiaim.angles.body_freestanding:override(false)
		slot_0_11_0.antiaim.angles.hidden:override(true)
	end)
	...
	pcall(function()
		rage.antiaim:inverter(elysian_heavy.body_flip)
		local hid = elysian_heavy.side and utils.random_float(70, 110) or utils.random_float(-110, -70)
		rage.antiaim:override_hidden_yaw_offset(hid)
		local hp = elysian_heavy.phase % 4 == 0 and utils.random_float(-89, 89) or 89
		rage.antiaim:override_hidden_pitch(hp)
	end)
...
events.createmove(function(cmd)
	elysian_heavy_apply(cmd)
end)

events.createmove_run(function(cmd)
	if elysian_heavy.exploit_was then
		elysian_heavy_apply(cmd)
	end
end)
...
pcall(function()
	...
	events.render(function()
		local cfg = slot_0_81_6 and slot_0_81_6.china_hat
		if not cfg or not cfg.switch or not cfg.switch:get() then return end
		if cfg.thirdperson_only and cfg.thirdperson_only:get() then
			local tp = false
			pcall(function() tp = slot_0_11_0.visuals.thirdperson:get() end)
		...
		local origin = nil
		pcall(function()
			if lp.get_hitbox_position then origin = lp:get_hitbox_position(0) end
		end)
		if not origin then
			pcall(function()
				local o = lp:get_origin()
				if o then origin = vector(o.x, o.y, o.z + 72) end
			end)
		end
		if not origin then return end
		origin = vector(origin.x, origin.y, origin.z + 2)
		local size = cfg.size:get()
		local height = cfg.height:get()
		local tip = vector(origin.x, origin.y, origin.z + height)
		local last = nil
		local base = cfg.color:get()
		local use_rgb = cfg.rgb:get()
		local fill_a = cfg.fill_a:get()
		local spd = cfg.speed:get()
		local style = "default"
		pcall(function() if cfg.style then style = cfg.style:get() end end)
		for i = 0, 360, 5 do
			local ang = math.rad(i)
			local pt = vector(origin.x - math.sin(ang) * size, origin.y - math.cos(ang) * size, origin.z)
			local r, g, b, a = base.r, base.g, base.b, base.a
			if use_rgb then
				r, g, b = hsv_to_rgb(((globals.realtime * (spd * 50)) + i) % 360 / 360, 1, 1)
			end
			if last then
				local s_old = render.world_to_screen(last)
				local s_new = render.world_to_screen(pt)
				local s_tip = render.world_to_screen(tip)
				if s_old and s_new and s_tip then
					if style == "solid" then
						pcall(function()
							render.poly(color(r, g, b, math.min(255, math.max(fill_a, 180))), s_old, s_new, s_tip)
						end)
						render.line(s_old, s_new, color(r, g, b, a))
					elseif style == "glass" then
						pcall(function()
							render.poly(color(r, g, b, math.min(90, fill_a)), s_old, s_new, s_tip)
						end)
						render.line(s_old, s_new, color(math.min(255, r + 40), math.min(255, g + 40), math.min(255, b + 40), math.floor(a * 0.7)))
						render.line(s_new, s_tip, color(255, 255, 255, math.floor(a * 0.25)))
					elseif style == "glow" then
						pcall(function()
							render.poly(color(r, g, b, math.floor(fill_a * 0.45)), s_old, s_new, s_tip)
						end)
						render.line(s_old, s_new, color(r, g, b, math.floor(a * 0.35)))
						local pt2 = vector(origin.x - math.sin(ang) * (size + 2), origin.y - math.cos(ang) * (size + 2), origin.z)
						local s_out = render.world_to_screen(pt2)
						if s_out then
							render.line(s_new, s_out, color(r, g, b, math.floor(a * 0.2)))
						end
						render.line(s_old, s_new, color(r, g, b, a))
					elseif style == "material" then
						local shade = 0.65 + 0.35 * math.abs(math.sin(ang * 3 + globals.realtime))
						local rr = math.floor(r * shade)
						local gg = math.floor(g * shade)
						local bb = math.floor(b * shade)
						pcall(function()
							render.poly(color(rr, gg, bb, math.min(255, fill_a + 40)), s_old, s_new, s_tip)
						end)
						render.line(s_old, s_new, color(math.floor(r * 0.5), math.floor(g * 0.5), math.floor(b * 0.5), a))
						render.line(s_new, s_tip, color(rr, gg, bb, math.floor(a * 0.8)))
					else -- default
						pcall(function()
							render.poly(color(r, g, b, fill_a), s_old, s_new, s_tip)
						end)
						render.line(s_old, s_new, color(r, g, b, math.min(255, a)))
					end
				end
			end
			last = pt
...
pcall(function()
	...
	pcall(function()
		a = ffi.cast(ffi.typeof("void***"), utils.create_interface("client.dll", "VClientEntityList003"))
		b = ffi.cast("get_client_entity_t", a[0][3])
		c = ffi.cast(ffi.typeof("void***"), utils.create_interface("engine.dll", "VModelInfoClient004"))
		d = ffi.cast("get_model_index_fn_t", c[0][2])
		e = ffi.cast("find_or_load_model_fn_t", c[0][43])
		f = ffi.cast(ffi.typeof("void***"), utils.create_interface("engine.dll", "VEngineClientStringTable001"))
		g = ffi.cast("find_table_t", f[0][3])
	end)
	if not a or not b or not c or not d or not e or not f or not g then return end

	local function precache_path(pa)
		local a_p = ffi.cast(ffi.typeof("void***"), g(f, "modelprecache"))
		if a_p == nil then return false end
		e(c, pa)
		local ac = ffi.cast("add_string_fn_t", a_p[0][8])
		if ac == nil then return false end
		if ac(a_p, false, pa, -1, nil) == -1 then return false end
		return true
	end

	local function set_model_index(en, i)
		local rw = b(a, en)
		if not rw then return end
		local gc = ffi.cast(ffi.typeof("void***"), rw)
		local se = ffi.cast("set_model_index_t", gc[0][75])
		if se ~= nil then se(gc, i) end
	end

	local function apply_model(ent, md)
		if not md or type(md) ~= "string" or #md <= 5 then return end
		if not precache_path(md) then return end
		local i = d(c, md)
		if i == -1 then return end
		set_model_index(ent, i)
	end

	local SAVE_PATH = "nl\\elysian\\models.json"
	local saved = slot_0_elysian_cm_saves or {}
	local active_path = "models/player/custom_player/legacy/tm_phoenix.mdl"

	local function persist()
		pcall(function()
			files.create_folder("nl\\elysian")
			files.write(SAVE_PATH, json.stringify(saved))
		end)
		slot_0_elysian_cm_saves = saved
	end

	local function find_by_name(name)
		for i = 1, #saved do
			if saved[i].name == name then return saved[i], i end
		end
		return nil, nil
	end

	pcall(function()
		if slot_0_elysian_cm_path then
			local v = slot_0_elysian_cm_path:get()
			if v and #v > 5 then active_path = v end
		end
	end)

	pcall(function()
		slot_0_elysian_cm_save_btn:set_callback(function()
			local name, path = "my model", active_path
			pcall(function()
				local n = slot_0_elysian_cm_name:get()
				if n and #n > 0 then name = n end
			end)
			pcall(function()
				local v = slot_0_elysian_cm_path:get()
				if v and #v > 5 then path = v end
			end)
			local entry, idx = find_by_name(name)
			if entry then saved[idx].path = path
			else saved[#saved + 1] = { name = name, path = path } end
			persist()
			active_path = path
		end)
	end)
	...
	pcall(function()
		slot_0_elysian_cm_del_btn:set_callback(function()
			local pick = nil
			pcall(function() pick = slot_0_elysian_cm_pick:get() end)
			if not pick or pick == "(path field)" then return end
			local _, idx = find_by_name(pick)
			if idx then table.remove(saved, idx) persist() end
		end)
	end)

	pcall(function()
		slot_0_elysian_cm_pick:set_callback(function()
			local pick = slot_0_elysian_cm_pick:get()
			if not pick or pick == "(path field)" then return end
			local entry = find_by_name(pick)
			if entry and entry.path then
				active_path = entry.path
				pcall(function() slot_0_elysian_cm_path:set(entry.path) end)
			end
		end)
	end)

	events.net_update_end(function()
		if not slot_0_elysian_custom_model or not slot_0_elysian_custom_model:get() then return end
		local lp = entity.get_local_player()
		...
		pcall(function()
			local pick = slot_0_elysian_cm_pick:get()
			if pick and pick ~= "(path field)" then
				local entry = find_by_name(pick)
				if entry and entry.path then path = entry.path end
			else
				local v = slot_0_elysian_cm_path:get()
				if v and #v > 5 then path = v end
			end
		...
		active_path = path
		pcall(function() apply_model(lp:get_index(), path) end)
	end)
end)

pcall(function()
	local st = { shots = 0, hits = 0, misses = 0, head_hits = 0, body_hits = 0, kills = 0, deaths = 0 }
	local pending, last_hg = {}, {}
	local DEATH_PENALTY = 10

	local function success_rate()
		if st.shots <= 0 then return 0 end
		local rate = (st.hits / st.shots) * 100 - st.deaths * DEATH_PENALTY
		if rate < 0 then rate = 0 end
		if rate > 100 then rate = 100 end
		return rate
	end

	local function hs_pct()
		if st.hits <= 0 then return 0 end
		return (st.head_hits / st.hits) * 100
	end

	local function refresh_labels()
		local s1 = string.format("shots %d  |  hits %d  |  miss %d", st.shots, st.hits, st.misses)
		local s2 = string.format("kills %d  |  deaths %d  |  hs %.0f%%", st.kills, st.deaths, hs_pct())
		local s3 = string.format("success %.0f%%", success_rate())
		pcall(function()
			if slot_0_84_13 and slot_0_84_13.stats1 then
				slot_0_84_13.stats1:name(s1)
				slot_0_84_13.stats2:name(s2)
				slot_0_84_13.stats3:name(s3)
			end
		end)
	end
	...
	pcall(function()
		if slot_0_84_13 and slot_0_84_13.stats_reset and slot_0_84_13.stats_reset.set_callback then
			slot_0_84_13.stats_reset:set_callback(function()
				st.shots, st.hits, st.misses, st.head_hits, st.body_hits, st.kills, st.deaths = 0, 0, 0, 0, 0, 0, 0
				pending, last_hg = {}, {}
				refresh_labels()
			end)
		end
	...
	events.weapon_fire(function(e)
		if not enabled() or not e then return end
		local me = entity.get_local_player()
		if not me then return end
		local shooter = nil
		pcall(function() shooter = entity.get(e.userid, true) end)
		if shooter ~= me then return end
		st.shots = st.shots + 1
		pending[#pending + 1] = { tick = globals.tickcount, resolved = false }
		refresh_labels()
	end)
	...
	events.player_hurt(function(e)
		...
		pcall(function()
			attacker = entity.get(e.attacker, true)
			victim = entity.get(e.userid, true)
		end)
		if attacker ~= me or not victim or victim == me then return end
		st.hits = st.hits + 1
		local hg = tonumber(e.hitgroup) or 0
		if hg == 1 then st.head_hits = st.head_hits + 1 else st.body_hits = st.body_hits + 1 end
		local vid = nil
		pcall(function() vid = victim.m_steamID or tostring(victim:get_index()) end)
		if vid then last_hg[vid] = hg end
		for i = #pending, 1, -1 do
			local age = globals.tickcount - (pending[i].tick or 0)
			if not pending[i].resolved and age >= 0 and age <= 16 then
	...
	events.player_death(function(e)
		...
		if attacker == me and victim and victim ~= me then
			st.kills = st.kills + 1
		end
		if victim == me then st.deaths = st.deaths + 1 end
		refresh_labels()
	end)
	...
	events.createmove(function()
		...
		for i = #pending, 1, -1 do
			local s = pending[i]
			if s.resolved or now - (s.tick or 0) >= 16 then
				if not s.resolved then st.misses = st.misses + 1 end
				table.remove(pending, i)
			end
		end
	end)

	events.round_start(function() pending = {} end)
	refresh_labels()
end)


do
	elysian_log("spark_init")
	local rand_f = utils.random_float
	local sin, cos, sqrt, floor = math.sin, math.cos, math.sqrt, math.floor
	local mmax, mabs = math.max, math.abs
	local pi2 = math.pi * 2
	local vec = vector
	local render_line = render.line
	local render_circ = render.circle

	local sparks, sparks_n = {}, 0
	local flashes, flashes_n = {}, 0
	local MAX_SPARKS = 250
	local FLASH_TIME = 0.06

	local function sp()
		return slot_0_81_6 and slot_0_81_6.spark_impact
	end
	local function on()
		local s = sp()
		return s and s.switch and s.switch:get()
	end
	...
	local function add_spark(x, y, z, vx, vy, vz, born, life)
		sparks_n = sparks_n + 1
		sparks[sparks_n] = { x = x, y = y, z = z, vx = vx, vy = vy, vz = vz, born = born, life = life }
	end
	...
	pcall(function()
		events.bullet_impact(function(e)
			if not on() then return end
			local ui = sp()
			if ui.self_only:get() then
				local me = entity.get_local_player()
				if not me then return end
			...
			if not ox then
				pcall(function()
					if e.pos then ox, oy, oz = e.pos.x, e.pos.y, e.pos.z end
				end)
			end
			...
			local base_speed = ui.speed:get()
			local base_life = ui.life:get() / 10

			if ui.flash:get() then
				flashes_n = flashes_n + 1
	...
	pcall(function()
		events.render(function()
			...
			if not dt or dt > 0.1 then dt = 0.016 end
			if dt > 0.1 then dt = 0.1 end

			local grav = ui.gravity:get() * dt
			local drag = 1 - 1.2 * dt
			if drag < 0 then drag = 0 end

			local c = ui.color:get()
			local cr, cg, cb = c.r, c.g, c.b
			local wr, wg, wb = 255 - cr, 255 - cg, 255 - cb

			local tail = ui.tail:get() / 100
			local thick = ui.thick:get()
			local flsz = ui.flash_sz:get()
			local flsz_inner = flsz * 0.35
			if flsz_inner < 1 then flsz_inner = 1 end
			local use_grad = ui.gradient:get()
			local use_3d = ui.perspective:get()
			local use_coll = ui.collide:get()
			local bounce = ui.bounce:get() / 100

			local cx, cy, cz
			if use_3d then
				local cam_pos = render.camera_position()
				if cam_pos then cx, cy, cz = cam_pos.x, cam_pos.y, cam_pos.z end
			end
			...
			while i <= flashes_n do
				...
				if age >= FLASH_TIME then
					...
					if scr then
						local aa = floor(255 * (1 - age / FLASH_TIME))
						render_circ(scr, color(cr, cg, cb, aa), flsz, 0, 1)
						render_circ(scr, color(255, 255, 255, aa), flsz_inner, 0, 1)
					end
					i = i + 1
				end
			end
				...
				if age >= s.life then
					...
					if use_coll then
						...
						if vlen2 > 900 then
							local tr
							pcall(function()
								tr = utils.trace_line(vec(s.x, s.y, s.z), vec(nx_pos, ny_pos, nz_pos), me_ent)
							end)
							if tr and tr.fraction and tr.fraction < 1 and tr.plane and not tr.all_solid then
					...
					s.x, s.y, s.z = nx_pos, ny_pos, nz_pos
					...
					if p_head and p_tail then
						local pscale = 1.0
						if use_3d and cx then
							local ddx, ddy, ddz = hx - cx, hy - cy, hz - cz
						...
						if use_grad then
							...
							for k = 1, SEGS do
								...
								if k == SEGS then
									...
									p_n = vec(tx_ + dx_w * t, ty_ + dy_w * t, tz_ + dz_w * t):to_screen()
								end
...
elysian_safe("crosshair_init", function()
	elysian_log("crosshair_init")
	events.render(function()
		local xh = slot_0_81_6 and slot_0_81_6.spin_crosshair
		if not xh or not xh.switch or not xh.switch:get() then return end
		local ss = render.screen_size()
		local cx, cy = ss.x * 0.5, ss.y * 0.5
		local col = xh.color:get()
		local sz = xh.size:get()
		local gap = xh.gap:get()
		local spd = xh.speed:get()
		local rot = (spd > 0) and (globals.realtime * (spd * 0.8)) or 0
		local c = color(col.r, col.g, col.b, col.a)
		local R = gap + sz

		local function pt(a, r)
			return vector(cx + math.cos(a) * r, cy + math.sin(a) * r)
		end
		local function tri(a0, radius)
			local p1 = pt(a0, radius)
			local p2 = pt(a0 + math.pi * 2 / 3, radius)
			local p3 = pt(a0 + math.pi * 4 / 3, radius)
			render.line(p1, p2, c)
			render.line(p2, p3, c)
			render.line(p3, p1, c)
		end
		...
		tri(rot - math.pi / 2, R)
		tri(rot - math.pi / 2 + math.pi / 3, R)
		render.circle(vector(cx, cy), c, 1.2, 0, 1)
	end)
end)


elysian_safe("post_init", function()
	elysian_log("=== script load finished ===")
end)


pcall(function()
	events.shutdown(function()
		elysian_log("=== script shutdown ===")
	end)
end)
...
pcall(function()
	local flakes = {}
	local max_default = 40
	local function rnd(a, b)
		return a + math.random() * (b - a)
	end
	local function spawn_one(ox, oy, oz, radius, height)
		local a = rnd(0, 6.28318530718)
		local d = rnd(0, radius)
		return {
			x = ox + math.cos(a) * d,
			y = oy + math.sin(a) * d,
			z = oz + rnd(40, height),
	...
	local snow_frame = 0
	events.render(function()
		local sn = slot_0_81_6 and slot_0_81_6.map_snow
		if not sn or not sn.switch or not sn.switch:get() then
			if #flakes > 0 then flakes = {} end
			return
		end
		snow_frame = snow_frame + 1
		local cnt = sn.count:get()
		if cnt > 40 and snow_frame % 2 == 0 then return end
		local lp = entity.get_local_player()
		if not lp then return end
		local origin
		pcall(function() origin = lp:get_origin() end)
		if not origin then return end

		local col = sn.color:get()
		local need = sn.count:get()
		if need > 80 then need = 80 end
		local radius = sn.radius:get()
		if radius > 1500 then radius = 1500 end
		local size = sn.size:get()
		if size > 4 then size = 4 end
		local fall = sn.speed:get() * 0.12
		local wind = sn.wind:get() * 0.05
		local height = math.max(160, radius * 0.35)
		local ox, oy, oz = origin.x, origin.y, origin.z

		while #flakes < need do
			flakes[#flakes + 1] = spawn_one(ox, oy, oz, radius, height)
		end
		while #flakes > need do
			flakes[#flakes] = nil
		end

		local ft = globals.frametime or 0.015
		if ft > 0.033 then ft = 0.033 end
		local step = ft * 55
		...
		for i = 1, #flakes do
			local f = flakes[i]
			f.ph = f.ph + ft
			f.x = f.x + (f.vx * wind + math.sin(f.ph) * wind * 0.5) * step
			f.y = f.y + (f.vy * wind) * step
			f.z = f.z + f.vz * fall * step

			local dx, dy = f.x - ox, f.y - oy
			if f.z < oz - 25 or (dx * dx + dy * dy) > r2 then
				flakes[i] = spawn_one(ox, oy, oz, radius, height)
				f = flakes[i]
			end

			local scr = render.world_to_screen(vector(f.x, f.y, f.z))
			if scr then
				render.circle(scr, col, size, 0, 1)
			end
		end
	end)

	events.round_start(function() flakes = {} end)
end)
